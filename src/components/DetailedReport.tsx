...
const DetailedReport: React.FC<DetailedReportProps> = ({ schools: initialSchools }) => {
  ...
  const totalCandidates = useMemo(() => filteredSchools.reduce((sum, school) => 
    sum + Object.values(school.candidates).reduce((schoolSum, count) => schoolSum + count, 0), 0
  ), [filteredSchools]);

  const totalFee = useMemo(() => filteredSchools.reduce((sum, school) => sum + calculateTotalFee(school.candidates), 0), [filteredSchools, licenseFees]);
      ...
      <table className={`min-w-full bg-white ${getFontSize()}`}>
          ...
          {filteredSchools.map((school) => {
            ...
            return (
              <tr key={school.id}>
                ...
                <td className="px-2 py-2 whitespace-nowrap font-medium text-green-600">{schoolTotalFee.toLocaleString('tr-TR')}</td>
              </tr>
            );
          })}
        </tbody>
        <tfoot className="bg-gray-50">
          <tr>
            ...
            {classTypes.map((classType) => (
              <td key={classType} className="px-2 py-2 whitespace-nowrap font-bold">
                {filteredSchools.reduce((sum, school) => sum + getCandidateCount(school, classType), 0)}
              </td>
            ))}
            <td className="px-2 py-2 whitespace-nowrap font-bold text-blue-600">{totalCandidates}</td>
            <td className="px-2 py-2 whitespace-nowrap font-bold text-green-600">{totalFee.toLocaleString('tr-TR')}</td>
          </tr>
        </tfoot>
      </table>
    </div>
